...
def run(args):
    for split in sorted(__import__("glob").glob("split_*.xyz")):
        ...
        proj = TSNE(n_components=2, random_state=args.seed).fit_transform(feats)
        sel = grid_select(proj, args.grid_division, args.seed)

        out = make_output_dir("selected_data_tsne", split[:-4])
        write(f"{out}/tsne_selection.xyz", [structures[i] for i in sel])
        plt.figure(figsize=(10, 6))
        plt.scatter(proj[:, 0], proj[:, 1], alpha=0.3)
        plt.scatter(proj[sel, 0], proj[sel, 1], alpha=0.7)
        plt.savefig(f"{out}/tsne_plot.png", dpi=300)
        print(f"[t-SNE] {split} → {len(sel)} frames → {out}")
